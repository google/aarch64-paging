...
impl<'a> UpdatableDescriptor<'a> {
    ...
    pub fn set(&mut self, pa: PhysicalAddress, flags: Attributes) -> Result<(), ()> {
        if !self.bbm_permits_update(Some(pa), flags) {
            return Err(());
        }
    ...
    fn bbm_permits_update(&self, pa: Option<PhysicalAddress>, flags: Attributes) -> bool {
        ...
        if pa.map_or(false, |p| p != self.output_address()) {
            return false;
        }
    ...
    pub fn modify_flags(&mut self, set: Attributes, clear: Attributes) -> Result<(), ()> {
        ...
        if !self.bbm_permits_update(None, flags) {
            return Err(());
        }
