...
impl LinearTranslation {
    ...
    pub fn new(offset: isize, ttbr: Ttbr) -> Self {
    ...
    fn virtual_to_physical(&self, va: VirtualAddress) -> Result<PhysicalAddress, MapError> {
        if (self.ttbr == Ttbr::Ttbr1) ^ ((va.0 as isize) < 0) {
...
impl LinearMap {
    ...
    ///
    /// The `offset` must be a multiple of [`PAGE_SIZE`]; if not this will panic.
    pub fn new(asid: usize, rootlevel: usize, offset: isize, ttbr: Ttbr) -> Self {
        Self {
            mapping: Mapping::new(LinearTranslation::new(offset, ttbr), asid, rootlevel, ttbr),
        }
    }
...
mod tests {
    ...
    fn map_valid() {
        ...
        let mut pagetable = LinearMap::new(1, 1, 4096, Ttbr::Ttbr0);
        assert_eq!(
        ...
        let mut pagetable = LinearMap::new(1, 1, LEVEL_2_BLOCK_SIZE as isize, Ttbr::Ttbr0);
        assert_eq!(
            pagetable.map_range(
    ...
    fn map_valid_negative_offset() {
        ...
        let mut pagetable = LinearMap::new(1, 1, -(PAGE_SIZE as isize), Ttbr::Ttbr0);
        assert_eq!(
            pagetable.map_range(
    ...
    fn physical_address_in_range_ttbr1() {
        ...
        let translation = LinearTranslation::new(GIB_512 + 4096, Ttbr::Ttbr1);
        assert_eq!(
            translation.physical_to_virtual(PhysicalAddress(8192)),
    ...
    fn block_mapping() {
        ...
        let mut pagetable = LinearMap::new(1, 1, 1 << 30, Ttbr::Ttbr0);
        pagetable
        ...
        let mut pagetable = LinearMap::new(1, 1, 1 << 29, Ttbr::Ttbr0);
        pagetable
            .map_range(&MemoryRegion::new(0, 1 << 30), Attributes::NORMAL)
            .unwrap();
        assert_eq!(
