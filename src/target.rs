...
use crate::paging::{attributes::Attributes, deallocate, PageTable, PhysicalAddress, Translation};
...
/// use aarch64_paging::{
///     paging::{
///         attributes::AttributesEl1, Constraints, MemoryRegion, PhysicalAddress, RootTable,
...
///     AttributesEl1::VALID
///         | AttributesEl1::ATTRIBUTE_INDEX_0
///         | AttributesEl1::INNER_SHAREABLE
///         | AttributesEl1::UXN,
///     Constraints::empty(),
/// )
/// .unwrap();
...
/// ```
#[derive(Debug)]
pub struct TargetAllocator<A: Attributes> {
    base_address: u64,
    allocations: Vec<Option<NonNull<PageTable<A>>>>,
}

impl<A: Attributes> TargetAllocator<A> {
...
impl<A: Attributes> Translation<A> for TargetAllocator<A> {
    ...
    unsafe fn deallocate_table(&mut self, page_table: NonNull<PageTable<A>>) {
        ...
        // and appropriate layout by calling `PageTable<A>::new()`.
        unsafe {
...
mod tests {
    ...
    fn map_one_page() {
        ...
        let bytes = map.translation().as_bytes();
        assert_eq!(bytes.len(), 3 * size_of::<PageTable<AttributesEl1>>());
        // Table mapping for table at 0x01_1000
        assert_eq!(
            bytes[0..8],
            [0x03, 0x10, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00]
        );
        for byte in &bytes[8..size_of::<PageTable<AttributesEl1>>()] {
            assert_eq!(*byte, 0);
        }
        // Table mapping for table at 0x01_2000
        assert_eq!(
            bytes[size_of::<PageTable<AttributesEl1>>()..size_of::<PageTable<AttributesEl1>>() + 8],
            [0x03, 0x20, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00]
        );
        for byte in &bytes
            [size_of::<PageTable<AttributesEl1>>() + 8..2 * size_of::<PageTable<AttributesEl1>>()]
        {
            assert_eq!(*byte, 0);
        }
        // Page mapping for 0x04_2000 with the attributes given above.
        assert_eq!(
            bytes[2 * size_of::<PageTable<AttributesEl1>>()
                ..2 * size_of::<PageTable<AttributesEl1>>() + 8],
            [0x03, 0x23, 0x04, 0x00, 0x00, 0x00, 0x40, 0x00]
        );
        for byte in &bytes[2 * size_of::<PageTable<AttributesEl1>>() + 8
            ..3 * size_of::<PageTable<AttributesEl1>>()]
        {
            assert_eq!(*byte, 0);
        }
    }
